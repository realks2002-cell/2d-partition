...
export function buildRenderPrompt(
  ...
  const placementDesc = placement
    ? `PLACEMENT PLACEHOLDER WITH INTERNAL STRUCTURE (CRITICAL — FOLLOW EXACTLY): The reference photo has been marked with neon UI colors showing exactly how to build the partition. YOU MUST READ THESE VISUAL MARKERS: (A) A BRIGHT MAGENTA semi-transparent ${shapeWord} marks the overall partition footprint in the photo's perspective. ${isQuad ? "The magenta shape is a 4-corner polygon that follows the perspective of the room (bottom edge = floor line, top edge = ceiling/top line). It is NOT an axis-aligned rectangle; it is projected to match the 3D perspective of the space." : ""} (B) Inside the shape, BRIGHT LIME GREEN VERTICAL LINES mark where each vertical steel mullion must go — count them and place a vertical mullion at each one, no more, no less. (C) If a BRIGHT LIME GREEN HORIZONTAL LINE exists inside the shape, it marks the ONE horizontal mullion position. If no lime horizontal line exists, there are ZERO horizontal mullions. (D) A BRIGHT CYAN semi-transparent COLUMN marks the door position — the swing door goes in that exact column. PROCEDURE: (1) Replace every pixel inside the magenta shape with a photorealistic glass partition. (2) Place vertical mullions exactly where the lime vertical lines are — same count, same positions. (3) Place the horizontal mullion exactly where the lime horizontal line is. If there is no lime horizontal line, use ZERO horizontal mullions. (4) Place the swing door exactly in the cyan highlighted column. (5) The partition in the output must fill the magenta shape pixel-for-pixel — do NOT shift, resize, rotate, or reposition. ${isQuad ? "Respect the trapezoidal/quadrilateral outline — the partition is a flat rectangle in 3D but projects to this shape in the photo's perspective." : ""} (6) ALL frame elements (vertical mullions, horizontal mullion if any, door frame, outer frame) MUST use ONE SINGLE uniform color: ${FRAME_COLOR_LABEL[frameColor]}. No white bars, no silver, no mixed colors. (7) COMPLETELY REMOVE all magenta, lime green, and cyan markings from the output. ZERO neon artifacts may remain. (8) The area OUTSIDE the magenta shape must remain IDENTICAL to the original reference photo pixels.`
    : "";
